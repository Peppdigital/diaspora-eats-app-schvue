...
export default function OrdersScreen() {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    console.log('[Orders iOS] Fetching orders for user:', user.id);
    setLoading(true);
    setError('');
    try {
      const data = await api.get('/api-orders');
      const list: ApiOrder[] = data?.orders || data || [];
      console.log('[Orders iOS] Loaded', list.length, 'orders');
      setOrders(list);
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : 'Failed to load orders';
      console.log('[Orders iOS] Fetch error:', msg);
      setError(msg);
    } finally {
      setLoading(false);
    }
            ...
            {filteredOrders.map((order) => {
              ...
              return (
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}
                  onPress={() => {
                    console.log('[Orders iOS] Order tapped:', order.id);
