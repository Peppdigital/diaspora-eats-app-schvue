// iOS-specific events screen — full implementation (no re-export to avoid circular import)
...
export default function EventsScreen() {
  ...
  const fetchEvents = useCallback(async () => {
    console.log('[Events iOS] Fetching events');
    setLoading(true);
    setError('');
    try {
      const data = await api.get('/api-events');
      const list: ApiEvent[] = data?.events || data || [];
      console.log('[Events iOS] Loaded', list.length, 'events');
      setEvents(list);
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : 'Failed to load events';
      console.log('[Events iOS] Fetch error:', msg);
      setError(msg);
    } finally {
      setLoading(false);
    }
  ...
  const EventCard = ({ event, featured = false }: { event: ApiEvent; featured?: boolean }) => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => {
          console.log('[Events iOS] Event tapped:', event.id, event.title);
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.filterButtonRow}>
          <TouchableOpacity
            ...
            onPress={() => {
              console.log('[Events iOS] Filters opened');
              setShowFilters(true);
            }}
            activeOpacity={0.7}
          >
          ...
          {dateFilter !== 'All' && (
            <TouchableOpacity style={[styles.clearButton, { backgroundColor: cardColor }]} onPress={() => setDateFilter('All')} activeOpacity={0.7}>
            ...
            {featuredEvents.length > 0 && (
              <View style={styles.section}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.horizontalList}>
                  {featuredEvents.map((event) => <EventCard key={event.id} event={event} featured />)}
                </ScrollView>
              </View>
            )}
            <View style={styles.section}>
              <Text style={[styles.sectionTitle, { color: textColor }]}>Upcoming Events ({filteredEvents.length})</Text>
              ...
              <View style={styles.filterGrid}>
                {dateFilters.map((filter) => (
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      console.log('[Events iOS] Date filter selected:', filter);
